...
def _seed_dirs(model, nodes, network, k, root='nets'):
    # `{k}_seed*` also matches the sibling CSVs LFC_props_csv_gen.py writes
...
def pooled_polarization(model, nodes, network, k, t_b, perc, root='nets'):
    '''
    Pools '<seed_dir>_{selector_label}.csv' (LTM_polspeed_seed_gen.py output)
    across every seed found for this (model, nodes, network, k). Same shape
    as the old single f'networks/{model}/{nodes}/{network}/{k}_{selector}{count}.csv'
    read (columns ID, network, p, th, seed, <cascade sizes 0.1..0.9>, no
    index set) - callers set_index(['ID','network','p','th','seed']) same as
    the original notebook cells do - plus one extra column, 'realization',
    not present in the original single-file schema (see below).
    ...
    timestamp stamped at generation time by LTM_net_gen_realizations.py),
    which can collide between two different seeds generated in the same
    millisecond under concurrent SLURM array submission. Confirmed via
    inventory: 9 such collisions out of 400 graph-writes in the mhk sweep
    (100 seeds x 4 p-values), each silently merging two distinct
    realizations' selected-node rows into one group wherever code grouped
    by 'ID' - undercounting e.g. p=0.0's realization count from 100 to 98.
    'realization' is just the (already-unique, filesystem-derived) seed
    directory name, so it can't collide the way a coarse timestamp can.
    '''
...
def pooled_eig_laplacians(model, nodes, network, k, root='nets', decimals=6, use_cache=True):
    ...
    (narval3), i.e. almost entirely Lustre I/O wait. Do not call this
