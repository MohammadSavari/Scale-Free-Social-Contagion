'''
Scans a directory tree for .gt files produced by LFC_net_gen_args.py and
...
generation script:
  ...
  - '<directory>_{t_b}_{perc}_corr_gains_degree.csv' for each (t_b, perc) in
    ...
    X% of nodes by degree, indexed by (ID, freq, p). Matches the naming
    Figure_generator.ipynb's LFC_plot expects
    (f'{k}_{t_b}_{perc}_corr_gains_{centrality}.csv').

LFC_net_gen_args.py only computes gains for the top 5% and bottom 5% of
nodes by degree (stored as the gains_top5 / gains_bot5 vertex properties),
not for every node, so only t_b:perc combinations matching what was
actually computed there (top:5, bot:5) will produce output here - anything
else is skipped with a warning. Node selection and gain averaging are both
derived from properties already stored on each .gt file - nothing is
recomputed via the expensive linear solves.

Example:
    python LFC_props_csv_gen.py --root nets
    python LFC_props_csv_gen.py --root nets --selectors top:5 bot:5
'''
...
def main():
    ...
    for path in Path(args.root).rglob('*.gt'):
        ...
        if k_filter is not None:
            # Directory name is '<k>_seed<seed>' (see LFC_net_gen_args.py); non-matching
    ...
    for directory, paths in sorted(by_dir.items()):
        ...
        # filename, which LFC_net_gen_args.py already makes genuinely unique
        # via a SLURM job/task or pid suffix) is used as the real ID instead.
