...
builds the per-directory CSVs the figure notebook reads:
...
def get_graph_props(G):
    ...
    return {
        ...
        # V/E/kbar make the exact-average-degree claim auditable straight
        # from the CSVs, with no .gt rescan: every row of an exact-degree
        # sweep must have kbar == k and V == N.
        'V': G.num_vertices(),
        'E': G.num_edges(),
        'kbar': 2 * G.num_edges() / G.num_vertices(),
    }
...
def main():
    ...
    for directory, paths in sorted(by_dir.items()):
        ...
        network_props = pd.DataFrame.from_dict(prop_rows, orient='index', columns=['CC', 'T', 'SP', 'l2', 'lmax_l2', 'Rg', 'V', 'E', 'kbar'])
