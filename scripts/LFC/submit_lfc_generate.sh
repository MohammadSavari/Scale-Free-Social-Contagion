...
# MEM / TIME (defaults derived from NODES - see the submission loop).
#
# LARGE N - the same path covers every node count, via NODES:
#   NODES=1000 bash submit_lfc_generate.sh mhk 1 1 16
#   NODES=5000 bash submit_lfc_generate.sh mhk 1 1 16
# This is affordable because net_functions.gain_all_nodes recovers H^2 for
# every node from a single eigendecomposition, so an N=5000 graph takes
# ~9.5 min rather than the ~22.6 h a per-node dense complex solve would
# need. CSVs follow the '<k>_seed<seed>_props.csv' naming throughout.
...
# The #SBATCH --mem/--time at the top of this file suit N=240. Cost is
# dominated by gain_all_nodes, which is O(N^3) in time and holds a handful of
# N x N dense doubles (~200 MB each at N=5000). Measured per graph:
# ~0.1 s at N=240, ~5 s at N=1000, ~9.5 min at N=5000. Defaults below carry
# generous headroom; override with MEM=... TIME=... if needed.
if [ -z "${MEM:-}" ] || [ -z "${TIME:-}" ]; then
    if   [ "$NODES" -le 300 ];  then MEM=${MEM:-2G};  TIME=${TIME:-0-00:10:00}
    elif [ "$NODES" -le 1200 ]; then MEM=${MEM:-4G};  TIME=${TIME:-0-00:30:00}
    else                             MEM=${MEM:-16G}; TIME=${TIME:-0-02:00:00}
    fi
fi
echo "NODES=${NODES}: requesting --mem=${MEM} --time=${TIME} per task"
...
for SEED in $(seq "$SEED_START" "$SEED_END"); do
    ...
    while [ "$START" -lt "$TOTAL_TASKS" ]; do
        ...
        sbatch --job-name="LFC_${MODEL_TYPE}_N${NODES}_seed${SEED}" --array="${START}-${END}%50" \
            --mem="${MEM}" --time="${TIME}" \
