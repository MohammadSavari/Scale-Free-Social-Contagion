...
- A `.ns_raw_cache/*.gt` glob-collision bug (the raw, unannotated
  Netzschleuder downloads cached there have no `ID`/`probability`/`category`
  graph properties, so a naive `**/*.gt` scan crashes on them) is already
  fixed in `extract_real_world_csv.py`/`generate_real_world_figure.py` -
  both skip any `.gt` file living under a dot-directory relative to the
  scan root.
...
../../nets/real_world/                      # data (populated by these scripts / unpack_data.sh)
